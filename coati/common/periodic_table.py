...
def PERIODIC_PROPERTY(property="cpk-hex"):
    return [Y[property] if property in Y else None for Y in PERIODIC_TABLE]


PERIODIC_MAX_X = max([ele["xpos"] for ele in PERIODIC_TABLE])
PERIODIC_MAX_Y = max([ele["ypos"] for ele in PERIODIC_TABLE])


@lru_cache(maxsize=None)
def XY_ONE_HOT_FULL(atomic_number):
    """
    A Version of XY one hot appropriate for the whole table.
    """
    tore = [0 for _ in range(PERIODIC_MAX_X + PERIODIC_MAX_Y)]
    xpos = PERIODIC_TABLE[atomic_number]["xpos"]
    ypos = PERIODIC_TABLE[atomic_number]["ypos"]
    tore[xpos] = 1
    tore[PERIODIC_MAX_X + ypos] = 1
    return tore


@lru_cache(maxsize=None)
def XY_ONE_HOT(atomic_number):
    if type(atomic_number) == type("H"):
        atomic_number = PERIODIC_PROPERTY("symbol").index(atomic_number)
    # row (oh 0...4), column (oh 0...7) No metals!
    # if xpos > 10, goes to xpos-10
    tore = [0 for _ in range(12)]
    xpos = PERIODIC_TABLE[atomic_number]["xpos"]
    ypos = PERIODIC_TABLE[atomic_number]["ypos"]
    if ypos >= 4 and xpos > 2 and xpos < 13:
        return tore
    if xpos < 0 or ypos < 0:
        return tore
    if xpos > 11:
    ...
    try:
        ...
        print(f"bad element {atomic_number} :: {xpos}, {ypos}")
    return tore
...
def IS_INORGANIC(an):
    """
    Gives the atomic number corresponding to the oh representation.
    """
    return all([not X for X in XY_ONE_HOT(an)])


def REVERSE_XY_ONE_HOT(oh):
    """
    Gives the atomic number corresponding to the oh representation.
    """
    codes = [tuple(XY_ONE_HOT(AN)) for AN in range(PERIODIC_TABLE[-1]["number"])]
    return codes.index(tuple(oh))


def XYZ_STRING(atoms, coords):
    tore = f"{int(sum(atoms>0))} 0 \n \n"
    for i in range(len(atoms)):
        if int(atoms[i]) > 0:
            tore = (
                tore
                + PERIODIC_TABLE[int(atoms[i])]["symbol"]
                + f" {coords[i,0]:.5f} {coords[i,1]:.5f} {coords[i,2]:.5f} \n"
            )
    return tore
